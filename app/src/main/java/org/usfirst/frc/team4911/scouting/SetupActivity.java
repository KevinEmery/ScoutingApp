...
public class SetupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edit_eventCode = (EditText)findViewById(R.id.editText_EventCode);
        ...
        edit_eventCode.setText(sharedpreferences.getString(EventCode, ""));
        ...
        String[] drive_stations = res.getStringArray(R.array.drive_stations_array);
        int position;
        for (position = 0; position < drive_stations.length; position++) {
        ...
        if (position < drive_stations.length)
        {
            adapter.setDropDownViewResource(position);
        }

        // Apply the adapter to the spinner
        spinner_driveStation.setAdapter(adapter);

        button_savePreferences.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String event_Code  = edit_eventCode.getText().toString();
